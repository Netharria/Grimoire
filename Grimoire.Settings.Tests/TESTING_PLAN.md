...
| Shouldly | Assertions |
| Microsoft.Extensions.Caching.Hybrid | Real in-memory HybridCache for each test class |
...
- `SettingsModuleFactory.Create(connectionString)` wires a fresh `HybridCache` + NSubstitute
  `IDbContextFactory<SettingsDbContext>` returning real contexts on each call.
