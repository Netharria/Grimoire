...
using Cybermancy.Core.Features.Moderation.Commands.LockCommands.LockChannel;
...
namespace Cybermancy.Discord.ModerationModule
{
    ...
    public class LockCommands : ApplicationCommandModule
    {
            ...
            [Option("Reason", "The reason why the channel is getting locked")] string? reason = null)
        {
            ...
            await ctx.SendLogAsync(response, CybermancyColor.Purple, $"{channel.Mention} has been locked for {durationAmount} {durationType.GetName()} by {ctx.User.Mention}");
        }
            ...
            [Option("Channel", "The Channel to unlock. Current channel if not specified.")] DiscordChannel? channel = null)
        {
            ...
            await ctx.SendLogAsync(response, CybermancyColor.Purple, $"{channel.Mention} has been unlocked by {ctx.User.Mention}");
        }
    }
}
