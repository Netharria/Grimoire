...
namespace Cybermancy.Discord.ModerationModule
{
    ...
    public class BanCommands : ApplicationCommandModule
    {
            ...
            [Option("DeleteDays", "Number of days of messages to delete. Default is 7")] long deleteDays = 7)
        {
            ...
            {
                await ctx.ReplyAsync(CybermancyColor.Orange, "I do not have permissions to ban that user.");
                return;
            }
            if(ctx.User.Id == user.Id)
            {
                await ctx.ReplyAsync(CybermancyColor.Orange, "You can't ban yourself.");
                return;
            }
