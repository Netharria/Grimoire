...
namespace Cybermancy.Core.Test.Unit
{
    public class TestDatabaseFixture
    {
        ...
        public CybermancyDbContext CreateContext()
        {
            var configuration = new ConfigurationBuilder()
            .SetBasePath(Directory.GetCurrentDirectory())
            .AddJsonFile("appsettings.json", optional: false, reloadOnChange: true)
            .Build();
            return new CybermancyDbContext(
                new DbContextOptionsBuilder<CybermancyDbContext>()
                    .UseNpgsql(configuration.GetConnectionString("CybermancyConnectionString"))
                    .Options);
        }
    }
}
