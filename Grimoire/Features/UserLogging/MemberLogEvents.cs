...
using Grimoire.Features.UserLogging.Commands;
using Grimoire.Features.UserLogging.Queries;
using Grimoire.Notifications;

namespace Grimoire.Features.UserLogging;
...
internal sealed class MemberLogEvents(IMediator mediator, IInviteService inviteService, IDiscordImageEmbedService imageEmbedService)
{
    ...
    public async Task DiscordOnGuildMemberRemoved(DiscordClient sender, GuildMemberRemovedEventArgs args)
    {
        var settings = await this._mediator.Send(new GetUserLogSettingsQuery{ GuildId = args.Guild.Id });
        if (!settings.IsLoggingEnabled) return;
    ...
    public async Task DiscordOnGuildMemberUpdated(DiscordClient sender, GuildMemberUpdatedEventArgs args)
    {
    ...
    private async Task ProcessNicknameChanges(GuildMemberUpdatedEventArgs args)
    {
    ...
    private async Task ProcessUsernameChanges(GuildMemberUpdatedEventArgs args)
    {
    ...
    private async Task ProcessAvatarChanges(GuildMemberUpdatedEventArgs args)
    {
