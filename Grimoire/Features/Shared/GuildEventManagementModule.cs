...
public sealed partial class GuildEventManagementModule(
    IMediator mediator,
    IInviteService inviteService,
    ILogger<GuildEventManagementModule> logger) : IEventHandler<GuildDownloadCompletedEventArgs>
...
//    IDiscordInviteDeletedEventSubscriber
{
    ...
    public async Task DiscordOnGuildDownloadCompleted(DiscordClient sender, GuildDownloadCompletedEventArgs args)
    {
        ...
        {
            Guilds = args.Guilds.Keys.ToArray().AsReadOnly(),
            ...
            Channels = args.Guilds.Values.SelectMany(x => x.Channels)
                .Select(x =>
                    new ChannelDto { Id = x.Value.Id, GuildId = x.Value.GuildId.GetValueOrDefault() }).Concat(args
    ...
    public async Task DiscordOnGuildCreated(DiscordClient sender, GuildCreatedEventArgs args)
    {
        ...
        await this._mediator.Send(new AddGuildCommand
        {
            ...
            Roles = args.Guild.Roles
                .Select(x =>
                    new RoleDto { GuildId = args.Guild.Id, Id = x.Value.Id }).ToArray().AsReadOnly(),
            Channels = args.Guild.Channels
                .Select(x =>
                    new ChannelDto { Id = x.Value.Id, GuildId = args.Guild.Id }).ToArray().AsReadOnly(),
