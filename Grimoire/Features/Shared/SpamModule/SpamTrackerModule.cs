...
{
    public ConcurrentDictionary<DiscordMember, SpamTracker> SpamUsers { get; set; } = new();
    ...
    public CheckSpamResult CheckSpam(DiscordMessage message)
    {
        ...
        if (AddPresure(spamTracker, message, message.Attachments.Count * 4.15))
            ...
            {
                IsSpam = true, Reason = "Automod: User Sent messages with several attachments"
            };
        ...
        if (AddPresure(spamTracker, message, message.Content.Split('\n').Length * 0.714))
            return new CheckSpamResult
            {
                IsSpam = true, Reason = "Automod: User sent too many new lines in a message."
            };
