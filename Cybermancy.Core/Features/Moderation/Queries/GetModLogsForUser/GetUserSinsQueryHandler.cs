...
namespace Cybermancy.Core.Features.Moderation.Queries.GetModLogsForUser
{
    public class GetUserSinsQueryHandler : IQueryHandler<GetUserSinsQuery, GetUserSinsQueryResponse>
    {
        private readonly ICybermancyDbContext _cybermancyDbContext;

        public GetUserSinsQueryHandler(ICybermancyDbContext cybermancyDbContext)
        {
        ...
        public async ValueTask<GetUserSinsQueryResponse> Handle(GetUserSinsQuery query, CancellationToken cancellationToken)
        {
            ...
            {
                queryable = this._cybermancyDbContext.Sins.Where(x => x.ModeratorId == query.UserId && x.GuildId == query.GuildId);
            }
            else
            {
                 queryable = this._cybermancyDbContext.Sins.Where(x => x.UserId == query.UserId && x.GuildId == query.GuildId);
            }
